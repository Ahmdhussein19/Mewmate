create or replace function public.is_finder_pet_photo_path(target_bucket text, target_path text)
returns boolean
language sql
security definer
stable
...
as $$
  select exists (
    ...
    where pet_photos.storage_bucket = target_bucket
      and pet_photos.storage_path = target_path
  );
$$;

revoke all on function public.is_finder_pet_photo_path(text, text) from public;
grant execute on function public.is_finder_pet_photo_path(text, text) to anon, authenticated;

drop policy if exists "finder can select pet photo objects" on storage.objects;

create policy "finder can select pet photo objects"
  on storage.objects for select
  to anon, authenticated
  using (
    bucket_id = 'pet-photos'
    and public.is_finder_pet_photo_path(storage.objects.bucket_id, storage.objects.name)
  );
